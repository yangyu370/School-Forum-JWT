...
这是一个基于 Spring Boot + Vue 3 的校园论坛系统，提供用户注册登录、帖子发布、评论互动、公告发布、通知推送等功能。
...
**接口地址**: `GET /api/forum/list-announcement`
  ...
  "data": [
    {
      "id": 1,
      "title": "论坛维护通知",
      "content": "{\"ops\":[{\"insert\":\"系统将于本周六进行维护，预计2小时\\n\"}]}",
      "time": "2024-01-01 12:00:00",
      "user": {
        "id": 1,
        "username": "管理员",
...
**说明**:
- 返回所有已发布的公告，按时间倒序排列
- `content` 为 Quill 编辑器的 JSON 格式
- 公告通常由管理员发布
- 前端可使用 `convertToHtml` 工具函数将 content 转换为 HTML 显示
...
**接口地址**: `POST /api/admin/forum/create-announcement`
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| title | string | 是 | 公告标题（1-30字符） |
| content | object | 是 | 公告内容（Quill Delta格式） |
...
```json
{
  ...
  "content": {
    "ops": [
      {
        "insert": "各位用户：\n系统将于本周六凌晨2:00-4:00进行维护升级，届时将无法访问论坛。\n请大家提前做好安排，感谢理解！\n"
      },
      {
        "attributes": {
          "bold": true
...
- 新增公告功能
  - 管理员可发布论坛公告
  - 用户可查看公告列表
  - 公告支持富文本格式（使用 Quill 编辑器）
  - 公告按时间倒序排列
- 新增接口
  - `POST /api/admin/forum/create-announcement` - 发布公告
  - `GET /api/forum/list-announcement` - 获取公告列表
- 优化
  - 公告内容支持图片、链接、代码块等富文本元素
  - 前端新增工具函数 `convertToHtml` 用于内容格式转换
